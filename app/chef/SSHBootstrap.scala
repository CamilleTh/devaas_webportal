    ...
    Akka.future{
      ...
      chefInfos match {
        case Some(chefRoles)=>
          ...
          val knifeCommand="knife bootstrap %s -r '%s' -j '{\"DeployList\": [{\"groupid\": \"%s\", \"artefactid\": \"%s\",\"version\": \"0.1-SNAPSHOT\",\"current_version\": \"none\",\"env\": \"%s\"}],\"proxy\": {\"hosts\": { \"%s\": \"%s\"}}, \"ci-server\": { \"jobs\" : { \"%s\" : \"http://10.241.97.11:8080/job/%s%s\"} }}'\n   -x root -P %s".format(destHost,chefRoles,groupId,appId,env,appId,"URL",env,appId,env,rootPassword)
